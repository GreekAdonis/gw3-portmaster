...
/* eglGetProcAddress override: logs every GL/EGL proc the engine requests
 * (the engine obtains ALL GL functions through this call) and returns the real
 * implementation. Used as a discovery harness for the soft-float↔hard-float
 * thunk layer. */
...
SOFTFP void glBlendColor_abi(float, float, float, float);
