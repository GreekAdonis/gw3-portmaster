...
int so_resolve(so_module *mod, so_default_dynlib *dynlib, int dynlib_size, int only_dynlib) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        /* Fallback: resolve from the host process (libc/libm/libstdc++/libdl/…).
         * The dynlib table above always wins when present (it is checked first),
         * so soft-float/thunked symbols still take precedence. This lets the
         * game and its companion .so files pull in standard C/C++ runtime symbols
         * without enumerating every one of them in the table. */
        if (!resolved) {
            void *h = dlsym(RTLD_DEFAULT, name_bare);
            if (h) resolved = (uintptr_t)h;
        }
